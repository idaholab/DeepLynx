...
{
    /// <inheritdoc />
    public partial class HistoricalRecordProvenanceUpdates : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            // Update the historical_records_trigger_func to include file hash
            migrationBuilder.Sql(@"
                ...
                BEGIN
                    ...
                    INSERT INTO deeplynx.historical_records (
                        ...
                        last_updated_by, last_updated_at, is_archived, file_type, file_size, file_content_hash,
                        labels, class_name, data_source_name, project_name, object_storage_name)
                    SELECT
                        ...
                        NEW.last_updated_by, NEW.last_updated_at, NEW.is_archived, NEW.file_type, NEW.file_size, NEW.file_content_hash,
                    ...
                    GROUP BY r.id, r.uri, r.name, r.description, r.properties, r.original_id, r.class_id, 
                        ...
                        r.last_updated_by, r.last_updated_at, r.is_archived, r.file_type, r.file_size, r.file_content_hash,
                        c.name, d.name, p.name, o.name;
                    RETURN NEW;
                END;
                $$ LANGUAGE plpgsql;
            ...
            // Update the historical_records_insert_association_trigger_func to include file hash
            ...
            migrationBuilder.Sql(@"
                ...
                BEGIN
                    ...
                    SELECT
                        ...
                        r.data_source_id, r.project_id, r.organization_id, r.object_storage_id,
                        json_agg(DISTINCT jsonb_build_object('id', t.id, 'name', t.name)) FILTER (WHERE t.id IS NOT NULL AND t.name IS NOT NULL),
                        r.last_updated_by, LOCALTIMESTAMP, r.is_archived, r.file_type, r.file_size, r.file_content_hash,
                        json_agg(DISTINCT jsonb_build_object('id', sl.id, 'name', sl.name)) FILTER (WHERE sl.id IS NOT NULL AND sl.name IS NOT NULL),
                        c.name, d.name, p.name, o.name
                    FROM deeplynx.records r
                    LEFT JOIN deeplynx.record_tags rt ON r.id = rt.record_id
        ...
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            ...
            // Recreate the separate functions from the previous migration (HistoricalRecordFileCols)
            // This matches what existed before HistoricalRecordProvenanceUpdates
            migrationBuilder.Sql(@"
                CREATE OR REPLACE FUNCTION deeplynx.historical_records_trigger_func()
                RETURNS TRIGGER AS $$
                BEGIN
