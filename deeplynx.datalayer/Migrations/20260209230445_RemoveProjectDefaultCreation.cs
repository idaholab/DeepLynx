...
namespace deeplynx.datalayer.Migrations
{
    ...
    // FIXED: Move pm.project_id reference to WHERE clause
    migrationBuilder.Sql(@"
        UPDATE deeplynx.project_members pm
    ...
    // Revert organization-level Admin role description
    migrationBuilder.Sql(@"
        UPDATE deeplynx.roles
        SET description = 'Admin role',
            last_updated_at = NOW()
        WHERE project_id IS NULL
          AND name = 'Admin';
    ");

    // Revert organization-level User role description
    migrationBuilder.Sql(@"
        UPDATE deeplynx.roles
        SET description = 'User role',
            last_updated_at = NOW()
        WHERE project_id IS NULL
          AND name = 'User';
    ");
