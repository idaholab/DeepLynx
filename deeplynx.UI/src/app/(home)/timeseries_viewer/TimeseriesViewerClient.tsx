...
import {
  getPlotData,
  getTimeseriesFiles,
} from "@/app/lib/client_service/olap_services.client";
...
export default function TimeseriesViewerClient({
  timeseriesFiles,
  organizationId,
  projectId,
}: Props) {
  ...
  const isReadyToLoad =
    activeFile !== null && selectedXAxis !== "" && selectedYAxes.length > 0;
  const [availableTimeseriesFiles, setAvailableTimeseriesFiles] =
    useState<HistoricalRecordResponseDto[]>(timeseriesFiles);
  ...
  const filteredTimeseriesData = React.useMemo(() => {
    ...
    const yAxisData = timeseriesData.filter((col) =>
      selectedYAxes.includes(col.name),
    );
  ...
  const handleExport = (format: "csv" | "png" | "pdf") => {
    if (format === "png") {
      const chartDiv = document.querySelector(
        ".echarts-timeseries-chart, .echarts-heatmap-chart",
      ) as HTMLDivElement;
      if (chartDiv) {
      ...
      const maxLength = Math.max(
        ...timeseriesData.map((item) => item.values.length),
      );
      ...
      alert(
        "PDF export requires additional library (jsPDF). Feature coming soon!",
      );
    }
  };
          ...
          <div className="card-body">
            <div className="flex justify-between">
              <h3 className="font-semibold mb-4">
                {t.translations.AVAILABLE_TIMESERIES_FILES}
              </h3>
              <button
                ...
                {timeseriesData[0]?.values.map((_, rowIndex) => (
                  <tr key={rowIndex}>
                    {timeseriesData.map((column, colIndex) => (
                      <td key={colIndex}>{column.values[rowIndex]}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
              ...
              <div className="card-body p-4">
                {!activeFile ? (
                  <div className="flex items-center justify-center min-h-96">
                    <div className="text-center">
                      <h3 className="text-xl font-semibold mb-2">
                        {t.translations.NO_FILE_SELECTED}
                      </h3>

                      <p className="text-base-content/60">
                        {t.translations.SELECT_FILE_TO_BEGIN}
                      </p>
                    </div>
                  </div>
                ) : loading ? (
                  ...
                  <div className="flex items-center justify-center min-h-96">
                    <div className="text-center">
                      <h3 className="text-xl font-semibold mb-2">
                        {t.translations.CONFIGURE_AXES}
                      </h3>

                      <p className="text-base-content/60">
                        {!selectedXAxis
                          ? t.translations.SELECT_X_AXIS_COLUMN
                          : t.translations.SELECT_Y_AXIS_COLUMN}
                      </p>
                    </div>
                  </div>
                ) : chartType === "2d-line" ? (
            ...
            <div className="card bg-base-100 shadow-xl">
              <div className="card-body p-4">
                ...
                <div className="mb-4">
                  <label className="label">
                    <span className="label-text font-medium">
                      {t.translations.LIMIT}: {limit}
                    </span>
                    <span className="label-text-alt">
                      {t.translations.MAX_DATA_POINTS}
                    </span>
                  </label>
                  <input
                    type="range"
                ...
                <div className="mb-4">
                  <label className="label">
                    <span className="label-text font-medium">
                      {t.translations.ROW_STRIDE}: {rowStride}
                    </span>
                    <span className="label-text-alt">
                      {t.translations.EVERY_NTH_ROW}
                    </span>
                  </label>
                  <input
                    type="range"
                  ...
                  <div className="form-control">
                    <label className="label cursor-pointer justify-start gap-2 py-1">
                      <input
                        ...
                        onChange={(e) =>
                          setDataZoom({ ...dataZoom, show: e.target.checked })
                        }
                        className="checkbox checkbox-primary checkbox-sm"
                      />
                      <div>
                        <span className="label-text text-sm font-medium">
                          {t.translations.ZOOM_CONTROLS}
                        </span>
                        <div className="text-xs text-base-content/60">
                          {t.translations.INTERACTIVE_ZOOM_SLIDER}
                        </div>
                      </div>
                    </label>
                  </div>
                  {chartType === "2d-line" && (
                    <div className="form-control">
                      <label className="label cursor-pointer justify-start gap-2 py-1">
                        ...
                        <div>
                          <span className="label-text text-sm font-medium">
                            {t.translations.MIN_MAX_MARKERS}
                          </span>
                          <div className="text-xs text-base-content/60">
                            {t.translations.SHOW_VALUE_MARKERS}
                          </div>
                        </div>
                      </label>
                    </div>
