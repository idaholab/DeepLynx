// src/app/api/project/[projectId]/logo/file/route.ts
...
export async function GET(
  ...
  try {
    ...
    for (const ext of possibleExtensions) {
      const filePath = path.join(logoDir, `project-${projectId}-logo.${ext}`);
      try {
    ...
    console.error("Error serving project logo:", error);
    return NextResponse.json(
      {
        message:
