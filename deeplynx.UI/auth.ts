...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  callbacks: {
    ...
    async session({ session, token }) {
      ...
      if (session.user) {
        ...
        session.user.username = (token.username || undefined) as
          | string
          | undefined;
  ...
  jwt: {
    encode: async (params: JWTEncodeParams) => {
      ...
      const { exp, ...tokenWithoutExp } = token as JWT & { exp?: number };
      return jsonWebToken.sign(tokenWithoutExp, signingSecret, {
        expiresIn: maxAge ?? 8 * 60 * 60,
      });
    },
