...
public class OrganizationController : ControllerBase
{
    ...
    /// <summary>
    /// Require Sensitivity Labels for all records in this organization
    /// </summary>
    /// <param name="organizationId"></param>
    /// <returns></returns>
    [HttpPost("{organizationId:long}/require-sensitivity-labels", Name = "api_require_sensitivity_labels")]
    [Auth("write", "organization")]
    public async Task<ActionResult> RequireSensitivityLabels(long organizationId)
    {
        try
        {
            await _organizationBusiness.RequireSensitivityLabels(organizationId);
            return Ok(new { message = $"Sensitivity Labels are now required for all records in organization with ID: {organizationId}" });
        }
        catch (Exception exc)
        {
            var message = $"An error occurred while making sensitivity labels required for all records in organization: {organizationId}: {exc}";
            _logger.LogError(message);
            return StatusCode(StatusCodes.Status500InternalServerError, message);
        }
    }
    ...
    /// <summary>
    /// unrequire Sensitivity Labels for all records in this organization
    /// </summary>
    /// <param name="organizationId"></param>
    /// <returns></returns>
    [HttpPost("{organizationId:long}/unrequire-sensitivity-labels", Name = "api_unrequire_sensitivity_labels")]
    [Auth("write", "organization")]
    public async Task<ActionResult> UnrequireSensitivityLabels(long organizationId)
    {
        try
        {
            await _organizationBusiness.UnrequireSensitivityLabels(organizationId);
            return Ok(new { message = $"Sensitivity Labels are now NOT required for all records in organization with ID: {organizationId}" });
        }
        catch (Exception exc)
        {
            var message = $"An error occurred while making sensitivity labels NOT required for all records in organization: {organizationId}: {exc}";
            _logger.LogError(message);
            return StatusCode(StatusCodes.Status500InternalServerError, message);
        }
    }
