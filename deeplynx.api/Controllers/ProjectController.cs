...
public class ProjectController : ControllerBase
{
    ...
    /// <summary>
    /// Require Sensitivity Labels for all records in this project
    /// </summary>
    /// <param name="organizationId"></param>
    /// <param name="projectId"></param>
    /// <returns></returns>
    [HttpPost("{projectId:long}/require-sensitivity-labels", Name = "api_require_sensitivity_labels")]
    [Auth("write", "project")]
    public async Task<ActionResult> RequireSensitivityLabels(long organizationId, long projectId)
    {
        try
        {
            await _projectBusiness.RequireSensitivityLabels(organizationId, projectId);
            return Ok(new { message = $"Sensitivity Labels are now required for all records in project with ID: {projectId}" });
        }
        catch (Exception exc)
        {
            var message = $"An error occurred while making sensitivity labels required for all records in project: {projectId}: {exc}";
            _logger.LogError(message);
            return StatusCode(StatusCodes.Status500InternalServerError, message);
        }
    }
    ...
    /// <summary>
    /// unrequire Sensitivity Labels for all records in this project
    /// </summary>
    /// <param name="organizationId"></param>
    /// <param name="projectId"></param>
    /// <returns></returns>
    [HttpPost("{projectId:long}/unrequire-sensitivity-labels", Name = "api_unrequire_sensitivity_labels")]
    [Auth("write", "project")]
    public async Task<ActionResult> UnrequireSensitivityLabels(long organizationId, long projectId)
    {
        try
        {
            await _projectBusiness.UnrequireSensitivityLabels(organizationId, projectId);
            return Ok(new { message = $"Sensitivity Labels are now NOT required for all records in project with ID: {projectId}" });
        }
        catch (Exception exc)
        {
            var message = $"An error occurred while making sensitivity labels NOT required for all records in project: {projectId}: {exc}";
            _logger.LogError(message);
            return StatusCode(StatusCodes.Status500InternalServerError, message);
        }
    }
