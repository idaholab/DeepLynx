...
public class DataSourceBusinessTests : IntegrationTestBase
{
    ...
    [Fact]
    public async Task GetDataSourceCount_ValidProjectId_ReturnsActiveCount()
    {
        // Act - pid has 1 active project-level + 1 org-level = 2
        var count = await _dataSourceBusiness.GetDataSourceCount(oid, new[] { pid });
    ...
    [Fact]
    public async Task GetDataSourceCount_HideArchived_ExcludesArchived()
    {
        // Act - pid has 1 active + 1 archived project-level + 1 org-level
        var countWithArchived = await _dataSourceBusiness.GetDataSourceCount(oid, new[] { pid }, hideArchived: false);
        var countWithoutArchived = await _dataSourceBusiness.GetDataSourceCount(oid, new[] { pid }, hideArchived: true);
    ...
    [Fact]
    public async Task GetDataSourceCount_NoProjectIds_ReturnsOrgLevelOnly()
    {
        // Act - org-level only (did2)
        var count = await _dataSourceBusiness.GetDataSourceCount(oid, null);
    ...
    [Fact]
    public async Task GetDataSourceCount_WithProjectIds_ReturnsProjectAndOrgLevel()
    {
        // Act - project-level (did) + org-level (did2)
        var count = await _dataSourceBusiness.GetDataSourceCount(oid, new[] { pid });

        // Assert - same as GetAllDataSources with same params
        var fullList = await _dataSourceBusiness.GetAllDataSources(oid, new[] { pid });
        Assert.Equal(fullList.Count, count);
    }

    [Fact]
    public async Task GetDataSourceCount_EmptyProject_ReturnsOrgLevelOnly()
    {
        // Act - pid2 has no project-level data sources, but org-level (did2) still included
        var count = await _dataSourceBusiness.GetDataSourceCount(oid, new[] { pid2 });

        // Assert - only the org-level data source
        Assert.Equal(1, count);
    }

    [Fact]
    public async Task GetDataSourceCount_InvalidOrgId_ReturnsZero()
    {
        // Act
        var count = await _dataSourceBusiness.GetDataSourceCount(999999, new[] { pid });
