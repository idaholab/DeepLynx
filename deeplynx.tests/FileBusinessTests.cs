...
public class FileBusinessTests : IntegrationTestBase
{
    ...
    /// <summary>
    ///     Configures the insight-business mock so an embed-enabled file/chunk upload can run
    ///     end-to-end: marks the file type as embeddable and resolves both the vlm and embedding
    ///     model configs used when building the provenance record.
    /// </summary>
    private (AiModelConfigResponseDto.WithToken Vlm, AiModelConfigResponseDto.WithToken Embedding) SetupEmbeddingModelMocks()
    {
        var vlmCfg = new AiModelConfigResponseDto.WithToken
        {
            Id = 101,
            ModelType = "vlm",
            ServerUrl = "http://vlm",
        ...
        var embCfg = new AiModelConfigResponseDto.WithToken
        {
            Id = 202,
            ModelType = "embedding",
            ServerUrl = "http://emb",
        ...
        _insightBusiness.Setup(i => i.IsSupportedFile(It.IsAny<string>())).Returns(true);
        _insightBusiness
            .Setup(i => i.ResolveModelConfig(It.IsAny<long>(), oid, pid, It.IsAny<long?>(), "vlm"))
            .ReturnsAsync(vlmCfg);
        _insightBusiness
            .Setup(i => i.ResolveModelConfig(It.IsAny<long>(), oid, pid, It.IsAny<long?>(), "embedding"))
            .ReturnsAsync(embCfg);
    ...
    [Fact]
    public async Task UploadFile_CreatesProvenanceRecord_WhenEmbedIsTrue()
    {
        var (_, embCfg) = SetupEmbeddingModelMocks();
        _provenanceBusiness
            .Setup(p => p.CreateProvenanceRecord(It.IsAny<long>(), "embedding_requested", uid, embCfg.Id))
            .ReturnsAsync(true);

        var file = CreateFormFile("embed me");

        var result = await _fileBusiness.UploadFile(
            uid, oid, pid, did, osid, file, embed: true);

        _provenanceBusiness.Verify(
            p => p.CreateProvenanceRecord(result.Id, "embedding_requested", uid, embCfg.Id),
            Times.Once);
    }

    [Fact]
    public async Task UploadFile_CreatesRecordButNotEmbeddingProvenance_WhenEmbedIsFalse()
    {
        var file = CreateFormFile("do not embed me");

        await _fileBusiness.UploadFile(uid, oid, pid, did, osid, file);

        // "create-record" should fire once, tied to the created record's id/user
        _provenanceBusiness.Verify(
            p => p.CreateProvenanceRecord(It.IsAny<long>(), "create-record", uid, null),
            Times.Once);

        // embedding-related provenance should never fire since embed was false
        _provenanceBusiness.Verify(
            p => p.CreateProvenanceRecord(It.IsAny<long>(), "embedding_requested", It.IsAny<long>(), It.IsAny<long?>()),
            Times.Never);

        // total call count guard, so nothing else sneaks in unnoticed
        _provenanceBusiness.Verify(
    ...
    [Fact]
    public async Task UploadFile_ReturnsSuccessfully_WhenProvenanceRecordCreationFails()
    {
        SetupEmbeddingModelMocks();
        // Simulate ProvenanceBusiness failing to find a historical record for the new upload
        _provenanceBusiness
            .Setup(p => p.CreateProvenanceRecord(It.IsAny<long>(), It.IsAny<string>(), It.IsAny<long>(), It.IsAny<long?>()))
            .ReturnsAsync(false);

        var file = CreateFormFile("embed me but provenance fails");

        // A failed provenance write is logged and swallowed — it must not block the upload
        var result = await _fileBusiness.UploadFile(uid, oid, pid, did, osid, file, embed: true);

        Assert.NotNull(result);
        _provenanceBusiness.Verify(
            p => p.CreateProvenanceRecord(result.Id, "embedding_requested", uid, It.IsAny<long?>()),
            Times.Once);
    }

    [Fact]
    public async Task UpdateFile_CreatesProvenanceRecord_WhenRecordIsEmbedded()
    {
        var originalFile = CreateFormFile("original content");
        var originalRecord = await _fileBusiness.UploadFile(uid, oid, pid, did, osid, originalFile);

        var trackedRecord = await Context.Records.FindAsync(originalRecord.Id);
        trackedRecord!.Embedded = true;
        await Context.SaveChangesAsync();
        
        _provenanceBusiness.Invocations.Clear(); // isolate from UploadFile's own create-record call

        var (_, embCfg) = SetupEmbeddingModelMocks();
        _provenanceBusiness
            .Setup(p => p.CreateProvenanceRecord(originalRecord.Id, "embedding_requested", uid, embCfg.Id))
            .ReturnsAsync(true);

        var newFile = CreateFormFile("updated content");
        await _fileBusiness.UpdateFile(uid, oid, pid, originalRecord.Id, newFile);

        _provenanceBusiness.Verify(
            p => p.CreateProvenanceRecord(originalRecord.Id, "embedding_requested", uid, embCfg.Id),
            Times.Once);

        _provenanceBusiness.Verify(
            p => p.CreateProvenanceRecord(originalRecord.Id, "update-record", uid, null),
            Times.Once);
    ...
    [Fact]
    public async Task UpdateFile_DoesNotCreateProvenanceRecord_WhenRecordIsNotEmbedded()
    {
        var originalFile = CreateFormFile("original content");
        var originalRecord = await _fileBusiness.UploadFile(uid, oid, pid, did, osid, originalFile);
        ...
        var newFile = CreateFormFile("updated content, still not embedded");
        await _fileBusiness.UpdateFile(uid, oid, pid, originalRecord.Id, newFile);

        _provenanceBusiness.Verify(
            p => p.CreateProvenanceRecord(It.IsAny<long>(), "update-record", It.IsAny<long>(), It.IsAny<long?>()),
            Times.Once);
    ...
    [Fact]
    public async Task CompleteUpload_CreatesProvenanceRecord_WhenEmbedIsTrue()
    {
        var (_, embCfg) = SetupEmbeddingModelMocks();
        _provenanceBusiness
            .Setup(p => p.CreateProvenanceRecord(It.IsAny<long>(), "embedding_requested", uid, embCfg.Id))
        ...
        var fileName = "embed-complete.txt";
        var initRequest = new FileUploadInitRequestDto { FileName = fileName, FileSize = 1024 };
        var session = await _fileBusiness.StartUpload(oid, pid, did, osid, initRequest);

        var chunk = CreateFormFile("embed-complete-content");
        await _fileBusiness.UploadChunk(oid, pid, did, osid, chunk, session.UploadId, 0);
        ...
        var result = await _fileBusiness.CompleteUpload(
            uid, oid, pid, did, osid, completeRequest, embed: true);
    ...
    [Fact]
    public async Task CompleteUpload_CreatesRecordButNotEmbeddingProvenance_WhenEmbedIsFalse()
    {
        var fileName = "no-embed-complete.txt";
        var initRequest = new FileUploadInitRequestDto { FileName = fileName, FileSize = 1024 };
        var session = await _fileBusiness.StartUpload(oid, pid, did, osid, initRequest);

        var chunk = CreateFormFile("no-embed-complete-content");
        await _fileBusiness.UploadChunk(oid, pid, did, osid, chunk, session.UploadId, 0);
    ...
    [Fact]
    public async Task UploadPartTus_CreatesProvenanceRecord_WhenFinalChunkCompletesWithEmbedTrue()
    {
        // Arrange
        const string uploadId = "test-upload-part-provenance";
        const long uploadOffset = 0;
        const long expectedNewOffset = 12;
        const long uploadLength = expectedNewOffset;
        ...
        innerFileBusiness
            .Setup(x => x.GetFileNameTus(
                ...
                uploadId,
                It.Is<ObjectStorageConfigDto>(c => c != null && c.MountPath == _testDirectory)))
            .ReturnsAsync("provenance-test-file.txt");

        var (_, embCfg) = SetupEmbeddingModelMocks();
        _provenanceBusiness
            .Setup(p => p.CreateProvenanceRecord(It.IsAny<long>(), "embedding_requested", currentUserId, embCfg.Id))
            .ReturnsAsync(true);
        ...
        // Assert
        Assert.Equal(expectedNewOffset, newOffset);

        var createdRecord = Context.Records.Single(r => r.Name == "provenance-test-file.txt");
        _provenanceBusiness.Verify(
            p => p.CreateProvenanceRecord(createdRecord.Id, "embedding_requested", currentUserId, embCfg.Id),
            Times.Once);
    }

    [Fact]
    public async Task UploadPartTus_CreatesRecordButNotEmbeddingProvenance_WhenEmbedIsFalse()
    {
        // Arrange
        const string uploadId = "test-upload-part-no-provenance";
        const long uploadOffset = 0;
        const long expectedNewOffset = 12;
        const long uploadLength = expectedNewOffset;
        ...
        innerFileBusiness
            .Setup(x => x.GetFileNameTus(
                ...
                uploadId,
                It.Is<ObjectStorageConfigDto>(c => c != null && c.MountPath == _testDirectory)))
            .ReturnsAsync("no-provenance-test-file.txt");
        ...
        // "create-record" should fire once, tied to the created record's id/user
        _provenanceBusiness.Verify(
            p => p.CreateProvenanceRecord(It.IsAny<long>(), "create-record", currentUserId, null),
            Times.Once);
