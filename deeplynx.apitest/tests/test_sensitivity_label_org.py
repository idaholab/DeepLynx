"""
DeepLynx Organization Labels API Tests

Tests all endpoints of the SensitivityLabelController API at the organization level.
"""
...
class TestOrgLabelsAPI:
    """Test suite for Organization Labels (Sensitivity Labels) API endpoints."""
    
    # ========================================================================
    # CREATE TESTS
    # ========================================================================
    
    def test_create_label_basic(self, client, organization):
        """Test creating a basic sensitivity label at organization level."""
        payload = {
            "name": "pytest_OrgBasicLabel",
            "description": "A basic test sensitivity label"
        }
        
        response = client.post(
            f"/organizations/{organization}/labels",
            json=payload
        )
        ...
        assert result.get("organizationId") == organization
        assert "isArchived" in result
        assert "lastUpdatedAt" in result or "last_updated_at" in result
        
        # Cleanup
        label_id = result.get("id")
        client.delete(f"/organizations/{organization}/labels/{label_id}")
    
    def test_create_label_minimal(self, client, organization):
        """Test creating a label with only required fields (name)."""
        payload = {
            "name": "pytest_OrgMinimalLabel"
        }
    ...
    def test_create_label_with_null_description(self, client, organization):
        """Test creating a label with explicit null description."""
        payload = {
            "name": "pytest_OrgNullDescLabel",
            "description": None
        }
    ...
    def test_create_label_empty_name_fails(self, client, organization):
    ...
    def test_create_label_missing_name_fails(self, client, organization):
    ...
    def test_create_multiple_labels(self, client, organization):
        """Test creating multiple sensitivity labels with different configurations."""
        label_configs = [
            {"name": "pytest_OrgConfidential", "description": "Confidential information"},
            {"name": "pytest_OrgInternal", "description": "Internal use only"},
            {"name": "pytest_OrgPublic", "description": "Publicly available information"}
        ]
        created_ids = []
    ...
    def test_create_duplicate_label_name(self, client, organization):
        """Test creating labels with duplicate names (if allowed)."""
        payload1 = {
            "name": "pytest_OrgDuplicateLabel",
        ...
        if response2.status_code == 200:
            ...
            client.delete(f"/organizations/{organization}/labels/{label_id_2}")
        
        # Cleanup first label
        client.delete(f"/organizations/{organization}/labels/{label_id_1}")
    
    # ========================================================================
    # READ TESTS
    # ========================================================================
    
    def test_get_all_labels_empty(self, client, organization):
        """Test getting all labels when none exist (or cleanup first)."""
        response = client.get(f"/organizations/{organization}/labels")
    ...
    def test_get_all_labels_with_data(self, client, organization):
        """Test getting all labels after creating some."""
        # Create multiple labels
        label_ids = []
        for i in range(3):
            payload = {
                "name": f"pytest_OrgListLabel_{i}_{int(time.time() * 1000)}",
                "description": f"Test sensitivity label {i}"
            }
            response = client.post(f"/organizations/{organization}/labels", json=payload)
    ...
    def test_get_all_labels_hide_archived_true(self, client, organization):
        """Test getting labels with hideArchived=true parameter."""
        # Create a label and archive it
        timestamp = int(time.time() * 1000)
        payload = {
            "name": f"pytest_OrgArchivedLabel_{timestamp}",
        ...
        archive_response = client.patch(
            f"/organizations/{organization}/labels/{label_id}",
            params={"archive": True}
        )
    ...
    def test_get_all_labels_hide_archived_false(self, client, organization):
        """Test getting labels with hideArchived=false parameter."""
        # Create a label and archive it
        timestamp = int(time.time() * 1000)
        payload = {
            "name": f"pytest_OrgShowArchivedLabel_{timestamp}",
    ...
    def test_get_label_by_id(self, client, organization):
        """Test getting a specific label by ID."""
        # Create a label
        payload = {
            "name": "pytest_OrgGetByIdLabel",
            "description": "Label for get by ID test"
        }
        
        create_response = client.post(f"/organizations/{organization}/labels", json=payload)
        assert create_response.status_code == 200
        label_id = create_response.json().get("id")
        
        # Get the label by ID
        response = client.get(f"/organizations/{organization}/labels/{label_id}")
    ...
    def test_get_label_nonexistent_id(self, client, organization):
        """Test getting a label with non-existent ID."""
        fake_id = 999999999
        
        response = client.get(f"/organizations/{organization}/labels/{fake_id}")
        
        # Should return 404 or similar
        assert response.status_code == 500
    
    def test_get_label_invalid_id(self, client, organization):
        """Test getting a label with invalid ID format."""
        invalid_id = "not-a-number"
        
        response = client.get(f"/organizations/{organization}/labels/{invalid_id}")
        
        # Should return 400 or 404
        assert response.status_code >= 400, "Invalid ID should return error"
    
    def test_get_label_with_hide_archived_param(self, client, organization):
        """Test getting a specific archived label with hideArchived parameter."""
        # Create and archive a label
        timestamp = int(time.time() * 1000)
        payload = {
            "name": f"pytest_OrgHideArchivedTest_{timestamp}",
    ...
    def test_update_label_name_and_description(self, client, organization):
        """Test updating both name and description of a label."""
        # Create a label
        payload = {
            "name": "pytest_OrgOriginalLabel",
    ...
    def test_update_label_name_only(self, client, organization):
        """Test updating only the name of a label."""
        # Create a label
        payload = {
            "name": "pytest_OrgNameUpdateLabel",
            "description": "Original description"
        }
        ...
        update_payload = {
            "name": "pytest_OrgUpdatedNameLabel",
            "description": None
        }
    ...
    def test_update_label_description_only(self, client, organization):
        """Test updating only the description of a label."""
        # Create a label
        payload = {
            "name": "pytest_OrgDescUpdateLabel",
            "description": "Original description"
        }
    ...
    def test_update_label_clear_description(self, client, organization):
        """Test clearing the description by setting it to null."""
        # Create a label with description
        timestamp = int(time.time() * 1000)
        payload = {
            "name": f"pytest_OrgClearDescLabel_{timestamp}",
    ...
    def test_update_label_empty_name_fails(self, client, organization):
        """Test that updating to empty name fails."""
        # Create a label
        payload = {
            "name": "pytest_OrgValidLabel",
        ...
        # Should fail
        assert response.status_code >= 400, "Empty name should not be allowed in update"
        
        # Cleanup
        client.delete(f"/organizations/{organization}/labels/{label_id}")
    
    def test_update_label_nonexistent_id(self, client, organization):
        """Test updating a label with non-existent ID."""
        fake_id = 999999999
        
        update_payload = {
            "name": "pytest_OrgNonexistentLabel",
            "description": "Should not work"
        }
        
        response = client.put(
            f"/organizations/{organization}/labels/{fake_id}",
            json=update_payload
        )
    ...
    def test_update_label_verify_timestamp(self, client, organization):
        """Test that updating a label updates the lastUpdatedAt timestamp."""
        # Create a label
        payload = {
            "name": "pytest_OrgTimestampLabel",
    ...
    def test_archive_label(self, client, organization):
        """Test archiving a label using PATCH with archive=true."""
        # Create a label
        timestamp = int(time.time() * 1000)
        payload = {
            "name": f"pytest_OrgArchiveLabel_{timestamp}",
            "description": "Label to be archived"
        }
        ...
        get_response = client.get(f"/organizations/{organization}/labels/{label_id}")
    ...
    def test_unarchive_label(self, client, organization):
        """Test unarchiving a label using PATCH with archive=false."""
        # Create and archive a label
        payload = {
            "name": "pytest_OrgUnarchiveLabel",
    ...
    def test_archive_label_nonexistent_id(self, client, organization):
    ...
    def test_delete_label(self, client, organization):
        """Test deleting a label."""
        # Create a label
        payload = {
            "name": "pytest_OrgDeleteLabel",
        ...
        # Delete the label
        response = client.delete(f"/organizations/{organization}/labels/{label_id}")
    ...
    def test_delete_label_nonexistent_id(self, client, organization):
        """Test deleting a label with non-existent ID."""
        fake_id = 999999999
        
        response = client.delete(f"/organizations/{organization}/labels/{fake_id}")
    ...
    def test_delete_label_twice(self, client, organization):
        """Test deleting the same label twice (idempotency)."""
        # Create a label
        payload = {
            "name": "pytest_OrgDoubleDeleteLabel",
        ...
        # Delete once
        response1 = client.delete(f"/organizations/{organization}/labels/{label_id}")
        assert response1.status_code == 200
        
        # Delete again
        response2 = client.delete(f"/organizations/{organization}/labels/{label_id}")
    ...
    def test_label_name_max_length(self, client, organization):
        """Test creating a label with very long name."""
        long_name = "pytest_org_" + "A" * 500  # Very long name
    ...
    def test_label_special_characters_in_name(self, client, organization):
    ...
    def test_label_unicode_characters(self, client, organization):
        """Test creating a label with Unicode characters."""
        payload = {
            "name": "pytest_OrgLabel_日本語_中文_العربية",
    ...
    def test_concurrent_label_creation(self, client, organization):
        ...
        for i in range(10):
            payload = {
                "name": f"pytest_OrgConcurrentLabel_{i}_{timestamp}",
    ...
    def test_label_organization_isolation(self, client, organization):
        """Test that labels are isolated to their organization."""
        # Create a label in the test organization
        payload = {
            "name": f"pytest_OrgIsolationLabel_{int(time.time() * 1000)}",
            "description": "Testing organization isolation"
        }
        ...
        # Verify it appears in the organization's labels list
        list_response = client.get(f"/organizations/{organization}/labels")
        assert list_response.status_code == 200
        labels = list_response.json()
        label_ids = [label.get("id") for label in labels]
        assert label_id in label_ids, "Label should appear in its organization"
        
        # Cleanup
        client.delete(f"/organizations/{organization}/labels/{label_id}")
    
    def test_complete_crud_lifecycle(self, client, organization):
        """Test complete CRUD lifecycle: create, read, update, archive, delete."""
        # CREATE
        timestamp = int(time.time() * 1000)
        payload = {
            "name": f"pytest_OrgLifecycleLabel_{timestamp}",
        ...
        # READ
        read_response = client.get(f"/organizations/{organization}/labels/{label_id}")
        assert read_response.status_code == 200
        assert read_response.json().get("name") == payload["name"]
        
        # UPDATE
        update_payload = {
            "name": f"pytest_OrgUpdatedLifecycleLabel_{timestamp}",
        ...
        # DELETE
        delete_response = client.delete(f"/organizations/{organization}/labels/{label_id}")
        assert delete_response.status_code == 200
        
        # VERIFY DELETION
        verify_response = client.get(f"/organizations/{organization}/labels/{label_id}")
        # API returns 500 instead of 404 for deleted labels
