"""Tests for Permission API endpoints at organization level."""
...
def test_create_permission_org(client, organization, cleanup_org_permissions):
    """Test creating a single permission at organization level."""
    payload = {
        "name": "pytest_OrgTestPermission",
        "description": "A test permission for organization-level API testing",
    ...
    if response.status_code == 200:
        cleanup_org_permissions.append(response.json()["id"])
    ...
    assert result["name"] == "pytest_OrgTestPermission"
    assert result["description"] == payload["description"]


def test_get_all_permissions_org(client, organization, cleanup_org_permissions):
    """Test retrieving all permissions at organization level."""
    payload = {
        "name": "pytest_OrgTestPermission",
        "description": "A test permission for organization-level API testing",
        "action": "permission testing with pytest"
    ...
    if create_response.status_code == 200:
        cleanup_org_permissions.append(create_response.json()["id"])
    ...
    assert our_permission["name"] == "pytest_OrgTestPermission"
    assert our_permission["description"] == "A test permission for organization-level API testing"


def test_get_permission_org(client, organization, cleanup_org_permissions):
    """Test retrieving a single permission by ID at organization level."""
    payload = {
        "name": "pytest_OrgTestPermission",
        "description": "A test permission for organization-level API testing",
        "action": "permission testing with pytest"
    ...
    get_response = client.get(
        f"/organizations/{organization}/permissions/{created_id}"
    )
    ...
    assert result["name"] == "pytest_OrgTestPermission"
    assert result["description"] == "A test permission for organization-level API testing"


def test_update_permission_org(client, organization, cleanup_org_permissions):
    """Test updating a permission at organization level."""
    # Create a permission
    create_payload = {
        "name": "pytest_OrgTestPermission",
    ...
    update_payload = {
        "name": "pytest_OrgTestPermission_Updated",
    ...
    assert result["id"] == created_id
    assert result["name"] == "pytest_OrgTestPermission_Updated"
    assert result["description"] == "Updated description"


def test_archive_and_unarchive_permission_org(client, organization, cleanup_org_permissions):
    """Test archiving and unarchiving a permission at organization level."""
    # Create a permission
    payload = {
        "name": "pytest_OrgTestPermission",
...
def test_delete_permission_org(client, organization, cleanup_org_permissions):
    """Test permanently deleting a permission at organization level."""
    # Create a permission
    payload = {
        "name": "pytest_OrgTestPermission",
    ...
    all_permissions_response = client.get(f"/organizations/{organization}/permissions")
