"""Tests for Project-level Role API endpoints."""

import pytest
import time

# ========================================================================
# PROJECT-LEVEL ROLE TESTS
# ========================================================================

def test_create_project_role(client, organization, project, cleanup_project_roles):
    """Test creating a single role at project level."""
    # Use timestamp to ensure unique names
    timestamp = int(time.time() * 1000)
    
    payload = {
        "name": f"pytest_ProjectTestRole_{timestamp}",
        "description": "A test role for project-level API testing"
    }

    response = client.post(
        f"/organizations/{organization}/projects/{project}/roles",
        json=payload
    )
    ...
    assert result["name"] == f"pytest_ProjectTestRole_{timestamp}"
    assert result["description"] == payload["description"]


def test_get_all_project_roles(client, organization, project, cleanup_project_roles):
    """Test retrieving all roles at project level."""
    # Use timestamp to ensure unique names
    timestamp = int(time.time() * 1000)
    ...
    assert our_role["name"] == f"pytest_ProjectTestRole_{timestamp}"
    assert our_role["description"] == "A test role for project-level API testing"


def test_get_project_role(client, organization, project, cleanup_project_roles):
    """Test retrieving a single role by ID at project level."""
    # Use timestamp to ensure unique names
    timestamp = int(time.time() * 1000)
    ...
    get_response = client.get(
        f"/organizations/{organization}/projects/{project}/roles/{created_id}"
    )
    ...
    assert result["name"] == f"pytest_ProjectTestRole_{timestamp}"
    assert result["description"] == "A test role for project-level API testing"


def test_update_project_role(client, organization, project, cleanup_project_roles):
    """Test updating a role at project level."""
    # Use timestamp to ensure unique names
    timestamp = int(time.time() * 1000)
    ...
    # Update the role
    update_payload = {
        "name": f"pytest_ProjectTestRole_Updated_{timestamp}",
    ...
    assert result["id"] == created_id
    assert result["name"] == f"pytest_ProjectTestRole_Updated_{timestamp}"
    assert result["description"] == "Updated description"


def test_archive_and_unarchive_project_role(client, organization, project, cleanup_project_roles):
    """Test archiving and unarchiving a role at project level."""
    # Use timestamp to ensure unique names
    timestamp = int(time.time() * 1000)
    ...
    archive_response = client.patch(
        f"/organizations/{organization}/projects/{project}/roles/{created_id}?archive=true"
    )
    ...
    get_archived_response = client.get(
        f"/organizations/{organization}/projects/{project}/roles/{created_id}?hideArchived=false"
    )
    ...
    unarchive_response = client.patch(
        f"/organizations/{organization}/projects/{project}/roles/{created_id}?archive=false"
    )
...
def test_delete_project_role(client, organization, project, cleanup_project_roles):
    """Test permanently deleting a role at project level."""
    # Use timestamp to ensure unique names
    timestamp = int(time.time() * 1000)
    ...
    all_roles_response = client.get(f"/organizations/{organization}/projects/{project}/roles")
...
def test_get_permissions_for_project_role(client, organization, project, cleanup_project_roles, cleanup_project_permissions):
    """Test getting permissions for a role at project level."""
    # Use timestamp to ensure unique names
    timestamp = int(time.time() * 1000)
...
def test_add_permission_to_project_role(client, organization, project, cleanup_project_roles, cleanup_project_permissions):
    """Test adding a permission to a role at project level."""
    # Use timestamp to ensure unique names
    timestamp = int(time.time() * 1000)
    ...
    # Create a permission
    perm_payload = {
        "name": f"pytest_TestPermission_{timestamp}",
    ...
    if perm_response.status_code == 200:
        cleanup_project_permissions.append(perm_response.json()["id"])
    ...
    add_response = client.post(
        f"/organizations/{organization}/projects/{project}/roles/{role_id}/permissions/{perm_id}"
    )
...
def test_set_permissions_for_project_role(client, organization, project, cleanup_project_roles, cleanup_project_permissions):
    """Test setting all permissions for a role (replaces existing) at project level."""
    # Use timestamp to ensure unique names
    timestamp = int(time.time() * 1000)
    ...
    for i in range(3):
        perm_payload = {
            "name": f"pytest_TestPermission_{timestamp}_{i+1}",
        ...
        if perm_response.status_code == 200:
            ...
            cleanup_project_permissions.append(perm_id)
...
def test_remove_permission_from_project_role(client, organization, project, cleanup_project_roles, cleanup_project_permissions):
    """Test removing a permission from a role at project level."""
    # Use timestamp to ensure unique names
    timestamp = int(time.time() * 1000)
